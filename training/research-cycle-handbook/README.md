...
- Mark each section label as a Markdown `####` heading with `.orc-card-section .unnumbered` (never a Bootstrap size utility such as `.fs-5`: Quarto copies a heading's classes onto its wrapping `<section>`, so a size utility there would also resize the whole card grid). `.unnumbered` keeps auto-numbering and the Word ToC out; add `.no-anchor` if the heading should not get an anchor link.
...
#### <i class="bi bi-mortarboard"></i> LEARN MORE {.orc-card-section .unnumbered}
...
#### <i class="bi bi-tools"></i> TOOLS & RESOURCES {.orc-card-section .unnumbered}
