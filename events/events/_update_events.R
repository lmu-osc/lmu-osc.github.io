...
files <- list.files("events/events", full.names = TRUE) %>%
  grep(".qmd", ., value = T)

events_new <- read.csv("events/events/_event_data_collector.csv")

# Function to overwrite existing YAML with updates
# This replaces existing values when they exist in updates
overwrite_lists <- function(existing, updates) {
  if (length(updates) == 0) {
    return(existing)
  }
  ...
  for (name in names(updates)) {
    if (is.list(updates[[name]]) && !is.null(names(updates[[name]])) && 
        name %in% names(result) && is.list(result[[name]]) && !is.null(names(result[[name]]))) {
      # Recursively overwrite nested lists
      result[[name]] <- overwrite_lists(result[[name]], updates[[name]])
    } else {
      # Overwrite the value
...
update_event_yaml <- function(file_path, csv_row) {
  ...
  # Overwrite existing YAML with updates (CSV values take precedence)
  merged_yaml <- overwrite_lists(existing_yaml, updates)
...
for(i in seq_len(nrow(events_new))) {
  ...
  file_path <- csv_row$file_paths_new
